...
function removeAllChildren(element) {
  while (element.firstChild) { // result-areaに、何かタグがあるかぎりループ
    // 子どもの要素があるかぎり削除
...
function appendAssessmentResult(element, result) {
  ...
  header.innerText = '診断結果'; //h3タグに「診断結果」という見出しを作成
  element.appendChild(header); //result-areaにheader変数を設定、div要素が親、headerが子
...
function appendTweetButton(element, message){
  ...
  const hrefValue =
    ...
    + '&ref_src=twsrc%5Etfw';
  anchor.setAttribute('href', hrefValue); //(属性,内容)
  anchor.className = 'twitter-hashtag-button'; //anchor.setAttribute('class', twitter-hashtag-button) でもOK
